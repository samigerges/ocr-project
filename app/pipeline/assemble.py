...
def assemble_results(doc_id: str,pages_dir: Path,ocr_dir: Path,out_dir: Path,) -> dict:
    ...
    for item in manifest:
        ...
        if source == "native":
            # native artifact is a text file in pages_dir
            ...
            # OCR artifact stored in ocr_dir/page_XXXX.json
            ocr_json = ocr_dir / f"page_{page_no:04d}.json"
            ...
            page_text = "\n".join([l.get("text", "") for l in lines if (l.get("text") or "").strip()]).strip()
